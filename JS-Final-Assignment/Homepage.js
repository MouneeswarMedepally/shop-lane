...
$(document).ready(function () {
  ...
  window.addEventListener('storage',(event)=>{
    if(event.key=='count'){
      cartCountSpan.innerText =localStorage.getItem("count")
    }
  })
  let filteredArray = [];
  $(".auto-play").slick({
    ...
    if (e.target.readyState === 4) {
      if (e.target.status === 200) {
        ...
        function onChange(e) {
          ...
          function searchFunction(item) {
            ...
            let includes =
              ...
              item.brand.toLowerCase().includes(trimmedSearchItem);
            return includes;
          }
        ...
        for (index = 0; index < productList.length; index++) {
          ...
          anchorCard.href = `./productdetails.html?id=${productDetails.id}`;
